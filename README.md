...
新节点先解码源视频以检测、跟踪和裁切人脸，修脸后输出原音视频 latent 与独立修复裁切数据，由“MiniMax H3 分块解码”分别解码并在最终 RGB 图像上贴回，不再把贴好的整段视频重新 VAE 编码。推荐顺序为“一采 → 可选高清二采 → 人脸修复 → Star7 分块解码”；上下文与最终解码使用同一个视频 VAE。普通 latent 解码器不会读取附带的修脸数据。旧版 IMAGE 节点继续在内部完成解码和贴回，保留原类 ID 并隐藏于普通节点搜索，兼容旧工作流。

恢复原有预设：自动平衡强度 0.30／取景 2.6，真人保真 0.25／2.8，远景小脸 0.48／2.4，动漫角色 0.32／2.7；沿用原 FaceRefine 跟踪、逐帧力度和贴回算法。已保存的“自定义”参数不会被覆盖，需要选择预设才会采用恢复后的参数。缺脸帧保留原时间线，修复帧数不匹配时明确报错。高强度仍可能改变身份或头部姿态，解码贴回不能修正已经生成的错误内容。

支持 1–4 人、主人物、画面中央和参考图匹配，以及自动平衡、真人保真、远景小脸、动漫角色和自定义预设。检测人数不足时自动按实际人数处理。参考图匹配需要可选的 InsightFace，其他模式不需要参考图。

“保持修复清晰度”默认开启：最终 RGB 贴回前，低于约 1.0MP 的底图会按原比例放大，例如 480×864 变为 768×1376；较大视频保持原尺寸。关闭时保留源尺寸，两种模式都不再整段 VAE 重编码。节点显示最终宽高与 MP，原始音频保持不变。
...
“高清放大模型”使用 `minimax_h3_latent_upscaler_3d_fp16.safetensors`。首次实际需要放大且文件缺失时，节点依次尝试 HF 国内镜像与 Hugging Face，校验固定 SHA-256 后写入 `ComfyUI/models/latent_upscale_models`；失败信息会列出目标目录和各下载源原因。当前只开放这份完整验证过的 FP16 模型，不会把共用目录里的 LTX 或同训练网络的 BF16/FP32 精度文件误当成不同画质模型。
