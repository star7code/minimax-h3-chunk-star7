...
| `MiniMax H3 多合一条件载入 - Star7` | 在一个节点中组织提示词、首尾帧、参考图、参考视频和音频，并输出可供人脸修复复用的上下文 |
...
普通 H3 工作流只需增加两处连接：使用“多合一条件载入”替代原条件节点，并把采样输出及其 `refine_context` 接入“一键人脸修复”。修复节点内部复用同一条已经完成 LoRA、Sigma Shift 与 Star7 分块处理的 MODEL，不需要再搭第二组模型加载、LoRA 或分块节点。文生视频无需参考图；选择“主人物”或“画面中央”时即使连接参考图也不会报错，参考图只在选择“参考图匹配”时参与身份选择。

默认包含自动平衡、真人保真、远景小脸、动漫角色和自定义预设，各预设会直接显示对应的修复步数、强度、尺寸、取景范围、融合强度和边缘羽化；修改任一数值会自动进入自定义，切换预设后再切回仍保留自定义值。默认 4 步针对 H3 Turbo，增加步数会直接增加二采时间，但不保证线性提升质量。第二行“修复人脸数量”可直接选择 1–4，默认 1；“目标人脸优先”在任何人数下都会保留：主人物按面积从大到小优先，画面中央按距离中心由近到远优先，参考图匹配先锁定参考图 1 的身份，再用主人物顺序补足其余名额。“主人物”并不理解剧情身份；硬切镜头后会重新选择，而参考身份会在新镜头重新匹配并跳过人物未出现的镜头。选择多人时只检测一次并建立相应数量的互斥镜头内轨迹，再逐脸完成局部采样和累积回贴；稳定检测数量不足时自动按实际人数修复并记录日志。每增加一张脸大致增加一份局部采样时间。参考图匹配需要可用的 InsightFace Python 运行库，缺失时会明确警告并回退到主人物；拥挤、人物交叉、侧脸、遮挡或极小脸场景仍可能匹配错误。

节点只重采检测到的人脸区域，音频 latent 只复制给 H3 的打包结构，不解码、不修改，最终输出可直接替代原视频 VAE 解码的 `IMAGE`。关闭修复或没有检测到可用人脸时会返回原始 VAE 解码画面。唯一额外必需模型是 `face_yolov8m.pt`：首次缺少时优先从 HF 国内镜像下载，随后回退到 Hugging Face；文件通过固定 SHA-256 校验后原子安装到 `ComfyUI/models/ultralytics/bbox`。无需额外人脸修复模型或超分模型，局部二采复用工作流现有 MiniMax H3 模型和视频/音频 VAE；InsightFace 仅是“参考图匹配”的可选增强，不会自动捆绑下载。

万能条件节点会把生成宽高自动吸附到最近的 32 倍数；输入图片在内部按对应画布处理。参考视频按 24 fps 使用，5 帧以上即可运行，短于 2 秒只提示，超过 15 秒或生成时长会自动裁切，并同步裁切同编号音轨与对齐到 H3 的 `17n+5` 帧网格。音频提供锁定原音、重混原音、仅作参考和模型原生生成；这里没有隐藏的音频增强模型，稳定性来自采样率转换、时长对齐和音频 latent 路由。

驱动音频使用唯一别名 `<Audio D>`。其余素材保持 H3 的正式顺序：已连接参考图依次为 `<Picture 1>`、`<Picture 2>`，参考视频依次为 `<Video 1>`、`<Video 2>`，参考视频配套音轨与独立参考音频从上到下共用 `<Audio 1>`、`<Audio 2>`。节点会根据实际接线在接口名称后动态显示标签，并在送入 H3 前把 `<Audio D>` 转换成模型所需的内部数字编号。

导演台会在单节点内部逐段完成条件构建、采样、二采和合并，外部只得到已经解码合并的画面，因此不能可靠地把普通 latent 精修节点插在它后面。当前版本不对导演台做伪接线；后续应通过其内部 `refine` 接口逐段接入，才能保留每段的条件与参考素材。
...
无需再创建子目录。V2 节点支持单张 `IMAGE` 和视频帧批次，默认“真实风格”；可选真实人像优化、3D 动漫、2D 动漫和自定义参数。容易放大皱纹、毛孔及压缩纹理的“真实风格加强”已经移除；写实与人像预设改用更克制的结构、色调和皮肤参数，优先保持人物年龄感与自然肤质。自定义参数在切换预设后仍会保留，并随工作流保存。“重置参数”会恢复当前预设的默认值；在自定义模式下恢复到真实风格的基准参数。

模型缺失时会在第一次执行节点时自动下载：优先使用 HF 国内镜像，失败后依次切换 Hugging Face 与 RankFTW GitHub 跨架构版本。下载先写入临时文件，经过文件大小、Windows PE 格式和固定 SHA-256 校验后再原子写入模型目录；失败会明确列出每个下载源的原因，并提示检查网络、代理、防火墙、磁盘空间或手动放置模型。不会留下可被误加载的半截 DLL。

“目标像素 (MP)”表示目标总像素量，例如 `2.0` 约为 200 万像素。节点按原图宽高比计算目标宽高；目标值不高于输入时不会降低分辨率，并会输出一条说明日志。需要放大时逐帧使用 Lanczos 高质量放大，再在目标尺寸执行 GPU Neural Rendering；逐帧处理和 CPU 批次输出可避免长视频放大后将整批帧重新压入显存。

“时序稳定”只在原生 Optical Flow 不可用的视频批次上生效。它对静止区域的 NR 增强残差做运动自适应平滑，并在运动边缘自动降低历史权重，以减少逐帧独立增强造成的闪烁而不过度制造拖影；单张图片和正常使用 Optical Flow 的路径不受影响。

当前已验证该 NR 运行库会忽略原生 `DLSSNR.Intensity` 和 `DLSSNR.Hint.Render.Preset`，但会响应 Style、局部色调、局部结构、皮肤结构和自动蒙版。因此界面的“NR 强度”由节点对“NR 输出与原图的差值”进行可验证的合成：`0` 保留放大后的原图，`1` 使用完整 NR 输出，`2` 将 NR 差异增强到两倍。节点不再暴露无效的内部 preset，也不再提供与 NR 强度重复的残差旋钮。

需要说明：仅使用 `nvngx_dlssnr.dll` 的路径是同分辨率 Neural Rendering。这里的放大是 Lanczos 与目标分辨率 NR 的组合，并非依赖独立 `nvngx_dlss.dll`、运动矢量、深度和曝光数据的原生 DLSS Super Resolution。V2 在支持 F16C 的 CPU 上使用硬件 FP16 上传与回读转换，不支持时自动回到原有兼容路径；SM80+ 在可用时启用 NVIDIA Optical Flow，SM75/Turing 使用稳定的独立帧路径，视频时序一致性可能略低。
