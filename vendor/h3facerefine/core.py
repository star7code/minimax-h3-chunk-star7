...
class H3FaceTrackCrop:
    ...
    def run(self, images, detector, confidence, crop_factor, canvas_width, canvas_height,
        ...
        chosen_indices = np.full(B, -1, dtype=np.int32)
            ...
            for pk in face_pick.get("picks") or []:
                if pk.get("absent"):
                    ...
                    if 0 <= _f < B and 0 <= _b < len(all_boxes[_f]):
                        forced[_f] = _b
            if face_pick.get("track_indices") is not None:
                explicit_track_indices = [int(value) for value in face_pick["track_indices"]]
                if len(explicit_track_indices) != B:
                    raise ValueError(
                        f"face_pick carries {len(explicit_track_indices)} track indices but {B} frames arrived"
                    )
                for _f, _b in enumerate(explicit_track_indices):
                    if 0 <= _b < len(all_boxes[_f]):
                        forced[_f] = _b
        ...
        for i in range(B):
            ...
            if i in absent_frames:
                continue
            if explicit_track_indices is not None and i not in forced:
                # A different face may still be visible during this lane's detection
                # dropout. Do not let continuity borrow that person's box.
                continue
            boxes = all_boxes[i]
            if not boxes:
                continue
            ...
            # Keep the detector-box index as a tiny private routing hint. Star7's
            # compact multi-face wrapper uses it to reserve a reference-matched
            # identity before filling the remaining requested slots.
            chosen_indices[i] = min(
                range(len(boxes)),
                key=lambda k: sum(abs(float(boxes[k][j]) - float(b[j])) for j in range(4)),
            )
        ...
        transform = {
            ...
            # Private in-memory cache used by Star7's compact multi-face mode. It
            # prevents each additional face pass from detecting the whole clip again.
            "tracking_cache": {
                "frames": int(B),
                "src_size": (int(W), int(H)),
                "boxes": all_boxes,
                ...
                "selected_indices": [int(value) for value in chosen_indices],
                "detector": detector,
...
class H3PerFrameDenoise:
    ...
    def run(self, model, av_latent, transform, denoise_multiplier_small_face, denoise_multiplier_large_face,
            ...
            scale_mode="absolute_px", verbose=True):
